...
lint-fix: golangci-lint ## Run golangci-lint linter and perform fixes
	GOOS=linux GOARCH=amd64 $(GOLANGCI_LINT) run --fix

.PHONY: lint-config
lint-config: golangci-lint ## Verify golangci-lint linter configuration
	GOOS=linux GOARCH=amd64 $(GOLANGCI_LINT) config verify
...
docker-build: ## Build standard Docker image (Alpine-based)
	$(CONTAINER_TOOL)  build --build-arg VERSION=$(VERSION) -t $(DRIVER_IMAGE):$(IMG_TAG) .

.PHONY: docker-push
docker-push: ## Push standard Docker image
	$(CONTAINER_TOOL)  push $(DRIVER_IMAGE):$(IMG_TAG)
...
push-latest: ## Push all images with 'latest' tag (required for integration tests and default deployments)
	$(CONTAINER_TOOL)  tag $(DRIVER_IMAGE):$(IMG_TAG) $(DRIVER_IMAGE):latest
	$(CONTAINER_TOOL)  push $(DRIVER_IMAGE):latest
	$(CONTAINER_TOOL)  tag $(OPERATOR_IMAGE):$(IMG_TAG) $(OPERATOR_IMAGE):latest
	$(CONTAINER_TOOL)  push $(OPERATOR_IMAGE):latest
	$(CONTAINER_TOOL)  tag $(BUNDLE_IMAGE):$(IMG_TAG) $(BUNDLE_IMAGE):latest
	$(CONTAINER_TOOL)  push $(BUNDLE_IMAGE):latest

.PHONY: operator-build
operator-build: ## Build the UBI-based operator image for Red Hat certification
	cp LICENSE operator/LICENSE
	cd operator && $(CONTAINER_TOOL)  build --pull -f Dockerfile.ubi --provenance=false --sbom=false --build-arg VERSION=$(VERSION) -t $(OPERATOR_IMAGE):$(IMG_TAG) .
	rm -f operator/LICENSE

.PHONY: operator-push
operator-push: ## Push the operator image
	$(CONTAINER_TOOL)  push $(OPERATOR_IMAGE):$(IMG_TAG)

.PHONY: bundle-build
bundle-build: ## Build the OLM bundle image
	cd operator && $(CONTAINER_TOOL)  build -f bundle.Dockerfile --provenance=false --sbom=false -t $(BUNDLE_IMAGE):$(IMG_TAG) .

.PHONY: bundle-push
bundle-push: ## Push the OLM bundle image
	$(CONTAINER_TOOL) push $(BUNDLE_IMAGE):$(IMG_TAG)
